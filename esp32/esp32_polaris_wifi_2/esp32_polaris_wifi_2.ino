/*
  ...
  MQTT live-streaming (added):
    - Publishes buoy/gps  at 1 Hz: lat/lon/alt/fix/siv/hacc/utc
    - Publishes buoy/status at 1 Hz: rtcm_bytes/reconnects/carrier/siv
    - Runs concurrently with NTRIP; broker is Mosquitto on your laptop
    - Add broker IP to secrets.h before flashing
  ...
  Before flashing, edit secrets.h:
    - Set mqttBroker to your laptop's LAN IP
    - Fill in WiFi and Polaris credentials
*/
...
unsigned long lastMqttPublish_ms = 0;
const unsigned long mqttPublishInterval_ms = 1000; // 1 Hz when NTRIP is off

// ============================================================
// MQTT helpers
// ============================================================

// Tries to connect once; non-blocking so it never stalls NTRIP.
void reconnectMQTT() {
  if (mqttClient.connected()) return;
  Serial.print(F("[mqtt] connecting to broker... "));
  if (mqttClient.connect("buoy")) {
...
// Publish GPS fix using the PVT data already cached by the most recent
// getPVT() call. Do NOT call getPVT() here — the caller must do it first.
void publishGPS() {
  if (!mqttClient.connected()) return;
  ...
  uint32_t hAccMm = myGNSS.getHorizontalAccEst(); // mm
  uint8_t h = myGNSS.getHour();
  uint8_t m = myGNSS.getMinute();
  uint8_t s = myGNSS.getSecond();

  // Translate to GGA quality: 0=no fix, 1=GPS, 4=RTK Fixed, 5=RTK Float
  int quality = 0;
  if (fix >= 2) {
  ...
  snprintf(payload, sizeof(payload),
    "{\"lat\":%.7f,\"lon\":%.7f,\"alt\":%.3f,"
    "\"fix\":%d,\"siv\":%d,\"hacc\":%.3f,"
    "\"utc\":\"%02d:%02d:%02d\"}",
    lat, lon, alt, quality, siv, hAccMm / 1000.0, h, m, s);

  mqttClient.publish("buoy/gps", payload);
}

// Publish session diagnostics alongside GPS.
void publishStatus() {
  if (!mqttClient.connected()) return;

  char payload[128];
  snprintf(payload, sizeof(payload),
    "{\"rtcm_bytes\":%lu,\"reconnects\":%d,\"carrier\":%d,\"siv\":%d}",
    rtcmTotalBytes, reconnectCount,
    (int)myGNSS.getCarrierSolutionType(), (int)myGNSS.getSIV());

  mqttClient.publish("buoy/status", payload);
}
...
void setup() {
  ...
  // MQTT — connect to local Mosquitto broker
  mqttClient.setServer(mqttBroker, mqttPort);
  mqttClient.setKeepAlive(30); // 30 s keepalive: tolerates blocking RTCM reads
  reconnectMQTT();
...
void loop() {
  ...
  // Keep MQTT connection alive and pump the TCP stack
  if (!mqttClient.connected()) reconnectMQTT();
  mqttClient.loop();

  // Publish GPS at 1 Hz while NTRIP is not running.
  // (While NTRIP runs, publishing happens inside beginClient()'s 1 Hz block.)
  if (!ntripRunning && millis() - lastMqttPublish_ms >= mqttPublishInterval_ms) {
    lastMqttPublish_ms = millis();
    myGNSS.getPVT();
    publishGPS();
  }
}
...
void beginClient() {
  ...
  while (ntripRunning) {
    ...
    // 1 Hz diagnostics + MQTT publish.
    // getPVT() is called once here; publishGPS() uses the cached values.
    if (millis() - lastDiag_ms > 1000) {
      ...
      // Reconnect MQTT if needed (non-blocking), then publish
      if (!mqttClient.connected()) reconnectMQTT();
      publishGPS();
      publishStatus();
    }
    ...
    // Pump the MQTT TCP stack. Called every loop iteration so the 15–30 s
    // keepalive is never missed even during heavy RTCM sessions.
    mqttClient.loop();
